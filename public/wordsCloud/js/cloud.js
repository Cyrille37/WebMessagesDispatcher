...
var background = svg.append("g"),
    vis = svg.append("g")
    .attr("transform", "translate(" + [layout_width >> 1, layout_height >> 1] + ")");
...
//hashchange('http://localhost/api.php/allwords?count=50');
hashchange( api_url );
...
function generate() {
  ...
  fontSize = d3.scale[d3.select("input[name=scale]:checked").property("value")]().range([30, 100]);
  if (tags.length)
...
function draw(data, bounds) {
  ...
  scale = bounds ? Math.min(
	layout_width / Math.abs(bounds[1].x - layout_width / 1),
	layout_width / Math.abs(bounds[0].x - layout_width / 2),
	layout_height / Math.abs(bounds[1].y - layout_height / 2),
	layout_height / Math.abs(bounds[0].y - layout_height / 2)) / 2 : 1;
  words = data;
  ...
  vis.transition()
      .delay(1000)
      .duration(750)
      .attr("transform", "translate(" + [layout_width >> 1, layout_height >> 1] + ")scale(" + scale + ")");
}
